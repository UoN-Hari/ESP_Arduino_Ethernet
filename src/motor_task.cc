...
void RxMsg_thread (void* pvParam) {
    ...
    for (;;) {
        ...
        if (Serial) {
            Serial.printf ("Angular: %.1f\n", twist.angular.z);
        }

        cmd_pos = twist.angular.z;
...
void MotorControl_thread (void* pvParam) {
    ...
    PID pid_vel (15.0f, 0.0f, 0.0f, 30000.0f, 2000.0f);
    PID pid_pos (0.6f, 0.02f, 0.05f, 300.0f, 50.0f);
    
    for (;;) {
        tx_msg.current[1] = pid_vel.calc (pid_pos.calc (cvt_to_ecd (cmd_pos + 180.0f, 8191), rx_msg.encoder), rx_msg.angular_vel);
